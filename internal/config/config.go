...
const appDir = "wendover"

// Directory functions

// CfgDir returns the directory where configuration filing and assets are stored.
// If the environment variable WENDOVER_CONFIG is set, then that configuration directory will be used.
// Otherwise, it defaults to ${UserConfigDir}/${appDir}
func CfgDir() (string, error) {
	cfgDir := os.Getenv("WENDOVER_CONFIG")
	if cfgDir != "" {
		return cfgDir, nil
	}
	...
	return filepath.Join(hd, appDir), nil
}

// CacheDir returns the directory where CAPWATCH and other cache filing are stored.
// If the environment variable WENDOVER_CACHE is set, then that configuration directory will be used.
// Otherwise, it defaults to ${UserCacheDir}/${appDir}
func CacheDir() (string, error) {
	cacheDir := os.Getenv("WENDOVER_CACHE")
	if cacheDir != "" {
		return cacheDir, nil
	}
	...
	return filepath.Join(cd, appDir), nil
}
